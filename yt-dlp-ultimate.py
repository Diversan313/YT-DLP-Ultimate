...
LOCALE = {
    'ru': {
        ...
        'btn_start': '▶ ЗАПУСТИТЬ',
        'btn_stop': '⏸ ПАУЗА',
        'btn_clear': '🗑 ОСТАНОВИТЬ И ОЧИСТИТЬ ВСЁ',
        ...
        'status_paused': '⏸ Пауза',
        'status_aborted': '⏹ Отменено',
        'status_merge': 'Конвертация в MP4...',
        ...
        'version': 'Версия: 1.1 (Browser Select)',
...
class ModernYouTubeCutter(ctk.CTk):
    def __init__(self):
        ...
        self.title("YT-DLP Ultimate v1.1")
    ...
    def ui_full_tab(self):
        ...
        self.combo_q_full.pack(anchor="w", padx=30, pady=(0, 40))
    ...
    def ui_frag_tab(self):
        ...
        ctk.CTkLabel(t, text=self.t('lbl_quality')).pack(anchor="w", padx=30, pady=(20, 0))
        vals = [self.t(k) for k in ['q_best', 'q_1080', 'q_720', 'q_audio']]
        self.combo_q_frag = ctk.CTkOptionMenu(t, values=vals, width=250)
        self.combo_q_frag.pack(anchor="w", padx=30, pady=(0, 20))
    ...
    def ui_queue_tab(self):
        ...
        self.btn_clear = ctk.CTkButton(ctrl, text=self.t('btn_clear'), fg_color="#C0392B", hover_color="#A93226", command=self.stop_and_clear_all)
    ...
    def ui_settings_tab(self):
        ...
        # Настройка Cookies с выбором браузера
        ctk.CTkLabel(t, text=self.t('sett_cookies')).pack(pady=(10, 5))
        self.cookies_val = ctk.StringVar(value="Disabled")
        # Отображаем на английском или русском - оставим технические названия
    ...
    def add_card(self, url, s, e, fmt, is_audio, q_lbl):
        ...
        self.download_queue.append({'id': tid, 'url': url, 's': s, 'e': e, 'fmt': fmt, 'is_audio': is_audio, 'done': False, 'error': None})
    ...
    def worker(self):
        ...
        for task in self.download_queue:
            ...
            # Применяем Cookies если выбран браузер
            if cookie_browser != "Disabled":
                # yt-dlp ожидает lower case (chrome, firefox, etc)
            ...
            try:
                ...
                if "ABORTED_BY_USER" in str(e):
                    ...
                    err_msg = str(e)
                    # Если ошибка куков, добавляем подсказку
